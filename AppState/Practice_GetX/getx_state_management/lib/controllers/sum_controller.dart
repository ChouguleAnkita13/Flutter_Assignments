...
class SumController extends GetxController {
  ...
  onInit() {
    ...
    ever(count1, (_) => log("$_ has been changed"));

    /// Called first time the variable $_ is changed
    once(count1, (_) => log("$_ was changed once"));

    /// Called once there has been no action on observable for 1 second
    debounce(count1, (_) => log("debouce$_"), time: const Duration(seconds: 1));

    /// Only reads the observable every 1 second
    interval(count1, (_) => log("interval $_"), time: const Duration(seconds: 1));
  }
}
